...
function() {
  function e() {
    ...
    function c(e) {
      ...
      if (e.target && t) {
        ...
        if (Tilda.sendEventToStatistics(i, o), "http" === c.substring(0, 4) && -1 === c.indexOf(".typeform.com/")) return window.setTimeout((function() {
  ...
  Tilda.sendEcommerceEvent = function(e, t) {
    ...
    for (var r, a = "",
    i = 0,
    o = [], c, d, n = "", m = "", l = "", u = 0; u < t.length; u++) {
      a > "" && (a += ", "),
      a += (d = t[u]).name,
      i += d.price,
      c = "",
      void 0 !== d.options && d.options.length > 0 && Array.prototype.forEach.call(d.options, (function(e) {
  ...
  Tilda.sendEventToStatistics = function(e, t, r, a) {
    ...
    o = [],
    c,
    d; (c = document.getElementById("allrecords")) && (d = null !== c.getAttribute("data-fb-event") ? c.getAttribute("data-fb-event") : window.jQuery && void 0 !== jQuery("#allrecords").data("fb-event") ? jQuery("#allrecords").data("fb-event") : null);
    var n = !(!d || "nosend" !== d),
    m = c ? c.getAttribute("data-vk-event") : null;
    c && (m = null !== c.getAttribute("data-vk-event") ? c.getAttribute("data-vk-event") : window.jQuery && void 0 !== jQuery("#allrecords").data("vk-event") ? jQuery("#allrecords").data("vk-event") : null);
    var l = !(!m || "nosend" !== m),
    u = "",
    ...
    r.event = "purchase";
    else if (r && r.ecommerce && (r.ecommerce.add && r.ecommerce.add.products ? o = r.ecommerce.add.products: r.ecommerce.remove && r.ecommerce.remove.products ? o = r.ecommerce.remove.products: r.ecommerce.detail && r.ecommerce.detail.products && (o = r.ecommerce.detail.products), o && o.length > 0)) {
      for (var s = 0; s < o.length; s++) o[s].id || (o[s].uid ? o[s].id = o[s].uid: o[s].sku ? o[s].id = o[s].sku: o[s].recid && o[s].lid && (o[s].id = o[s].recid + "_" + o[s].lid));
    ...
    try {
      ...
      if (window.gtagTrackerID && "gtag" === window.mainTracker) if (i) if (r && r.event) if ("purchase" === r.event) {
        for (var o = r.ecommerce.purchase.products,
        w = 0,
        g = 0; g < o.length; g++) if ("delivery" === o[g].id) {
          w = o[g];
          break
        }
        gtag("event", "purchase", {
    ...
    if (window.ga && "tilda" !== window.mainTracker && "gtag" !== window.mainTracker) if (i) if (r && r.event) {
      try {
        if (window.Tilda.isLoadGAEcommerce || (window.Tilda.isLoadGAEcommerce = !0, ga("require", "ec")), ga("set", "currencyCode", u), "purchase" === r.event) {
          for (var f, y = r.ecommerce.purchase.products.length,
          g = 0; g < y; g++) f = r.ecommerce.purchase.products[g],
          ga("ec:addProduct", {
            id: f.id || g,
            name: f.name,
          ...
          for (var f, y = r.ecommerce.add.products.length,
          g = 0; g < y; g++) f = r.ecommerce.add.products[g],
          ga("ec:addProduct", {
            id: f.id || g,
            name: f.name,
            price: parseFloat(f.price).toFixed(2),
          ...
          for (var f, y = r.ecommerce.remove.products.length,
          g = 0; g < y; g++) f = r.ecommerce.remove.products[g],
          ga("ec:addProduct", {
            id: f.id || g,
            name: f.name,
            price: parseFloat(f.price).toFixed(2),
          ...
          for (var f, y = r.ecommerce.detail.products.length,
          g = 0; g < y; g++) f = r.ecommerce.detail.products[g],
          ga("ec:addProduct", {
            id: f.id || g,
            name: f.name,
            price: parseFloat(f.price).toFixed(2),
    ...
    if (window.mainMetrikaId && window.mainMetrikaId > 0 && "function" == typeof ym) if (i) {
      ...
      u && (h.params.currency = u), ym(window.mainMetrikaId, "hit", e, h)) : ym(window.mainMetrikaId, "hit", e, h)
    } else a > 0 ? (h = {
      order_price: a
    },
    u && (h.currency = u), ym(window.mainMetrikaId, "reachGoal", e, h)) : ym(window.mainMetrikaId, "reachGoal", e);
    if (window.mainMetrika > "" && window[window.mainMetrika] && (i ? a > 0 ? window[window.mainMetrika].hit(e, {
      ...
      if (i) if ( - 1 === e.indexOf("tilda/") || -1 === e.indexOf("/payment/") && -1 === e.indexOf("/submitted/")) if (r && r.event && a > 0) if ("addToCart" === r.event && r.ecommerce.add) {
        for (var f, y = r.ecommerce.add.products.length,
        k = [], g = 0; g < y; g++) f = r.ecommerce.add.products[g],
        k.push(f.id || f.uid || f.name);
        ...
        for (var f, y = r.ecommerce.detail.products.length,
        k = [], g = 0; g < y; g++) f = r.ecommerce.detail.products[g],
        k.push(f.id || f.uid || f.name);
    ...
    if (void 0 !== window.VK && void 0 !== window.VK.Retargeting && !1 === l) try {
      if (i) {
        ...
        "purchase" === r.event && r.ecommerce.purchase) if (a > 0 && b > 0) {
          var f;
          A.currency_code = u;
          for (var y = r.ecommerce.purchase.products.length,
          k = [], g = 0; g < y; g++) f = r.ecommerce.purchase.products[g],
          A.products.push({
        ...
        else if ("addToCart" === r.event && r.ecommerce.add) if (a > 0 && b > 0) {
          var f;
          A.currency_code = u;
          for (var y = r.ecommerce.add.products.length,
          k = [], g = 0; g < y; g++) f = r.ecommerce.add.products[g],
          A.products.push({
            id: f.id || f.uid || f.name,
            price: f.price ? f.price: 0
          ...
          A.total_price = a;
          T = "add_to_cart"
        } else T = "t-add-to-cart",
        r.ecommerce.add[0] && r.ecommerce.add[0].uid && (T += "-" + r.ecommerce.add[0].uid);
        else if ("viewProduct" === r.event && r.ecommerce.detail) if (a > 0 && b > 0) {
          var f;
          A.currency_code = u;
          for (var y = r.ecommerce.detail.products.length,
          k = [], g = 0; g < y; g++) f = r.ecommerce.detail.products[g],
          A.products.push({
            id: f.id || f.uid || f.name,
            price: f.price ? f.price: 0
          ...
          A.total_price = a;
          T = "view_product"
        } else T = "t-view-product",
        r.ecommerce.detail[0] && r.ecommerce.detail[0].uid && (T += "-" + r.ecommerce.detail[0].uid);
        else if ("removeFromCart" === r.event && r.ecommerce.remmove) if (a > 0 && b > 0) {
          var f;
          A.currency_code = u;
          for (var y = r.ecommerce.remove.products.length,
          k = [], g = 0; g < y; g++) f = r.ecommerce.remove.products[g],
          A.products.push({
            id: f.id || f.uid || f.name,
            price: f.price ? f.price: 0
          }),
          A.total_price = a;
          T = "remove_from_cart"
        } else T = "t-remove-product",
        r.ecommerce.remove[0] && r.ecommerce.remove[0].uid && (T += "-" + r.ecommerce.remove[0].uid);
        else T = r.event;
        else if ( - 1 !== e.indexOf("tilda/") && -1 !== e.indexOf("/payment/")) T = "t-purchase-" + (F = (F = (F = e.replace("/tilda/", "")).replace("tilda/", "")).replace("/payment/", ""));
        else if ( - 1 !== e.indexOf("tilda/") && -1 !== e.indexOf("/submitted/")) T = "t-lead-" + (F = (F = (F = e.replace("/tilda/", "")).replace("tilda/", "")).replace("/submitted/", ""));
        ...
        b > 0 && A && A.currency_code ? (VK.Retargeting.Event("purchase"), VK.Retargeting.ProductEvent(b, T, A)) : (VK.Retargeting.Event(T), "t-purchase" === T.substring(0, 10) ? VK.Goal("purchase") : "t-lead" === T.substring(0, 6) && VK.Goal("lead"))
    ...
    "function" == typeof window.tildastat && (i ? (e.indexOf("payment") > 0 && e.indexOf("tilda/form") > -1 && (e = e.replace("tilda/form", "tilda/rec")), window.tildastat("pageview", {
      page: e
    ...
    try {
      ...
      if ( - 1 !== e.toLowerCase().indexOf("utm_") && "string" == typeof(t = (t = (e = e.toLowerCase()).split("?"))[1])) {
        ...
        if (r.length > 0) {
          ...
          document.cookie = "TILDAUTM=" + encodeURIComponent(r) + "; path=/; expires=" + c.toUTCString()
        }
      }
    } catch(d) {}
  },
